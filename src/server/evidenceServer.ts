...
const allowedOrigins = (process.env.CORS_ORIGIN || 'https://sweater9.github.io')
  .split(',')
...
function isEvidenceItem(value: unknown): value is ScreeningEvidenceItem {
  ...
  return typeof item.id === 'string' && !!item.id.trim()
    ...
    && typeof item.retrievedAt === 'string' && !!item.retrievedAt.trim()
...
export function createEvidenceServer() {
  ...
  app.get('/api/health', async (_req, res) => {
    ...
    res.status(nvidia.configured && nvidia.reachable ? 200 : 503).json({
      status: nvidia.configured && nvidia.reachable ? 'ok' : 'degraded',
      timestamp: new Date().toISOString(),
      screeningEngine: 'NVIDIA_NIM_EVIDENCE_BOUND',
      nvidia: {
        configured: nvidia.configured,
        reachable: nvidia.reachable,
        model: nvidia.model,
        error: nvidia.error,
      },
    });
  ...
  app.post('/api/screen', async (req, res) => {
    try {
      ...
      const evidence = (Array.isArray(rawEvidence) ? rawEvidence : []).filter(isEvidenceItem);
      const analysis = await analyzeWithNvidia(subject, evidence);
      return res.json({
        reportId: `VER-${Date.now().toString(36).toUpperCase()}`,
        generatedAt: new Date().toISOString(),
        investigator: 'VeritasScreen NVIDIA NIM Evidence Analysis',
        subject,
      ...
      return res.status(502).json({
        error: 'Screening analysis could not be completed.',
...
if (process.env.NODE_ENV !== 'test') {
  createEvidenceServer().listen(PORT, '0.0.0.0', () => {
    console.log(`VeritasScreen evidence API listening on port ${PORT}`);
  });
}
